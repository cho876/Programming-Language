/*
 *	 Project #2
	제목 PhoneContactPolyLMV에 PhoneContactVector 추가

	내용

	1. 주어진 PhoneContactPolyLMV 사용, PhoneContactVectorClass 수정
	2. project#1의 추가한 find, update 여기서도 추가
	3. Map에서 insert, update에서 error가 나오게 수정
	4. 초기 nameData와 numberData, emailData에 자기만의 data 5개 추가,listAll()
	5. project#1의 데이터 그대로 테스트 데이터로 사용
 */
