/*
 * 	제목 : TemplateQuickSort 프로그램 작성

	내용
		5장의 카드를 sort한 후 사전순으로 비교
 */
...
void sortFiveCards(List<FiveCards>& L){
	FiveCards five;
	List<FiveCards> fiveList;
	for (Iterator<FiveCards> cp = L.begin(); cp!=L.end(); cp.next()) {
		five = cp.get();
		five.sort();
		fiveList.push_back(five);
	}
	L = fiveList;
}
